...
df_wsls <- data.frame(
  ...
  Estimated_Value = c(p_stay_win_est, p_shift_lose_est)
)

#compiles, but questionable outcome why do each look identical?
draws_df <- as_draws_df(fit$draws()) 

ggplot(draws_df, aes(.iteration,p_stay_win, group = .chain, color = .chain)) +
  geom_line(alpha = 0.5) +
  theme_classic()

ggplot(draws_df, aes(.iteration, p_shift_lose, group = .chain, color = .chain)) +
  geom_line(alpha = 0.5) +
  theme_classic()
